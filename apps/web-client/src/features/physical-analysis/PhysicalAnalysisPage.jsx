...
export default function PhysicalAnalysisPage() {
    ...
    // ─── Diagnostic logging ──────────────────────────────────
    // This will reveal why some charts show fewer days than expected.
    useEffect(() => {
        if (!daily.length) return;
        const counts = {
            total: daily.length,
            withSleep: daily.filter((d) => d.sleep_hours > 0).length,
            withSteps: daily.filter((d) => d.steps > 0).length,
            withEnergy: daily.filter((d) => d.active_energy > 0).length,
            withAllThree: daily.filter((d) =>
                d.sleep_hours > 0 && d.steps > 0 && d.active_energy > 0
            ).length,
            dateRange: `${daily[0]?.date} → ${daily[daily.length - 1]?.date}`,
        };
        console.log('🔍 [PhysicalAnalysis] daily data report:', counts);
        const incomplete = daily.filter((d) =>
            !(d.sleep_hours > 0 && d.steps > 0 && d.active_energy > 0)
        );
        if (incomplete.length) {
            console.log(`🔍 [PhysicalAnalysis] ${incomplete.length} incomplete days (missing one or more metrics):`,
                incomplete.map((d) => ({
                    date: d.date,
                    sleep: d.sleep_hours, steps: d.steps, energy: d.active_energy,
                })));
        }
    }, [daily]);
