...
export default function BurnoutTimeline({ timeline }) {
    const insight = useMemo(() => {
        if (!timeline || timeline.length < 5) return null;

        const burnoutScores = timeline.map(d => d.burnout);
        const sleepScores   = timeline.map(d => d.sleep);

        const recent = burnoutScores.slice(-3);
        const earlier = burnoutScores.slice(0, Math.max(3, Math.floor(burnoutScores.length / 2)));
        const recentAvg = recent.reduce((a, b) => a + b, 0) / recent.length;
        const earlierAvg = earlier.reduce((a, b) => a + b, 0) / earlier.length;
        const delta = recentAvg - earlierAvg;

        const burnoutDays = timeline.filter(d => d.burnout >= 56).length;
        const totalDays   = timeline.length;

        let trend;
        if (delta > 8)       trend = 'rising — your burnout signal is trending upward';
        else if (delta < -8) trend = 'easing — your burnout signal is trending downward';
        else                 trend = 'holding steady';

        const sleepLow = sleepScores.filter(s => s < 60).length;
        const sleepNote = sleepLow >= totalDays * 0.4
            ? ` Sleep is consistently low (${sleepLow}/${totalDays} days below 60), which is amplifying the load.`
            : '';

        const riskNote = burnoutDays > 0
            ? ` ${burnoutDays} of ${totalDays} days crossed into the burnout-risk zone.`
            : ` No days crossed into the burnout zone — your load is well-managed.`;

        return `Pattern is ${trend}.${riskNote}${sleepNote}`;
    }, [timeline]);
    ...
    return (
        <div className="burnout-timeline">
            <Plot
                data={[
                    {
                        x: dates, y: sleepScores, type: 'scatter', mode: 'lines', name: 'Sleep',
                        line: { color: 'rgba(183, 148, 244, 0.5)', width: 1.5, dash: 'dot' },
                        hovertemplate: '<b>Sleep:</b> %{y}/100<br>%{x}<extra></extra>',
                    },
                    {
                        x: dates, y: stressScores, type: 'scatter', mode: 'lines', name: 'Stress',
                        line: { color: 'rgba(251, 146, 60, 0.6)', width: 1.5, dash: 'dot' },
                        hovertemplate: '<b>Stress:</b> %{y}/100<br>%{x}<extra></extra>',
                    },
                    {
                        x: dates, y: burnoutScores, type: 'scatter', mode: 'lines+markers', name: 'Burnout',
                        line: { color: '#0BEFC4', width: 3, shape: 'spline', smoothing: 0.8 },
                        marker: { color: '#0BEFC4', size: 6, line: { color: '#0BEFC4', width: 2 } },
                        fill: 'tozeroy',
                ...
                layout={{
                    ...
                    shapes: [
                        { type: 'rect', xref: 'paper', yref: 'y', x0: 0, x1: 1, y0: 0,  y1: 30,
                          fillcolor: 'rgba(52, 211, 153, 0.04)', line: { width: 0 }, layer: 'below' },
                        { type: 'rect', xref: 'paper', yref: 'y', x0: 0, x1: 1, y0: 30, y1: 55,
                          fillcolor: 'rgba(245, 200, 66, 0.04)', line: { width: 0 }, layer: 'below' },
                        { type: 'rect', xref: 'paper', yref: 'y', x0: 0, x1: 1, y0: 55, y1: 75,
                          fillcolor: 'rgba(251, 146, 60, 0.05)', line: { width: 0 }, layer: 'below' },
            ...
            {/* Trend insight */}
            {insight && (
                <p className="burnout-timeline__insight">{insight}</p>
            )}
        </div>
    );
}
